...
import GridListFunc from './gridList';
...
{/* {props.userDetail!==undefined */}
	...
	<Grid item md={6} sm={12} xs={12}>
		<TextField fullWidth label="Status" className={classes.textField}
		// value={userObj.FirstName}
			// onChange={(event) => setObjKeyAttribute({ ...userObj, FirstName: event.target.value })} 
            />
	</Grid>
	<Grid item md={6} sm={12} xs={12}>
		<TextField fullWidth label="Sales Channel" className={classes.textField}
		// value={userObj.LastName}
			// onChange={(event) => setObjKeyAttribute({ ...userObj, LastName: event.target.value })} 
            />
	</Grid>
	<Grid item lg={4} md={4} sm={12} xs={12}>
		<TextField fullWidth label="Brand Name" className={classes.textField}
		// value={userObj.Email}
			// onChange={(event) => setObjKeyAttribute({ ...userObj, Email: event.target.value })} 
            />{' '}
	</Grid>
	<Grid item lg={4} md={4} sm={12} xs={12}>
		<TextField fullWidth label="Asset Category" className={classes.textField}
    ...
    <Grid item lg={12} xs={12}>
		<input fullWidth label="Choose Image" className={classes.textField} type = "file"
		// value={userObj.Address}
			// onChange={(event) => setObjKeyAttribute({ ...userObj, Address: event.target.value })}
		/>
	</Grid>
...
<Grid container spacing={2} justify="flex-start">
	<Grid item>
		<Button alignItems="center" variant="contained" color="default"
			// onClick={() => props.GetUsertoRole(props.DistId)}
            >Cancel</Button>
	</Grid>
	{/* <Grid item>
	...
	<Grid item>
		<Button alignItems="center" variant="contained" color="primary"
			// onClick={() => props.UpdateUsertoRole(userObj,props.DistId,props.userDetail.userRoleId)}
            >Save
	</Button>
	</Grid>
</Grid></Paper>
...
export default withRouter(ViewPlanogram)